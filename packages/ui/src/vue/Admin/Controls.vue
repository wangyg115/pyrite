<template>
    <nav class="c-admin-controls">
        <RouterLink
            ...
            :to="{name: 'admin-users-user-settings', params: {userId: $s.admin.user.id, tabId: 'misc'}}"
        >
        ...
        <RouterLink
            v-if="$s.admin.group"
            class="btn btn-menu tooltip"
            :class="{active: $route.name.startsWith('admin-groups')}"
            :data-tooltip="$t('Groups')"
            :to="{name: 'admin-groups-group-settings', params: {groupId: $s.admin.group._name, tabId: 'misc'}}"
        >
